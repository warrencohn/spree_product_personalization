...
describe Spree::LineItemPersonalization do
  ...
  let(:product_personalization) { variant.product.product_personalization }

  def get_params(value)
    options  = { line_item_personalization_attributes: { value: value } }
    ActionController::Parameters.new(options).permit(:line_item_personalization_attributes => Spree::LineItemPersonalization.permitted_attributes)
  end
  ...
  it "adds line_item to the order" do
    ...
    expect(line_item.line_item_personalization).to be
    expect(line_item.line_item_personalization.value).to eq(value_1)
    expect(line_item.line_item_personalization.name).to eq(product_personalization.name)
    expect(line_item.line_item_personalization.price).to eq(product_personalization.calculator.preferred_amount)
  end
